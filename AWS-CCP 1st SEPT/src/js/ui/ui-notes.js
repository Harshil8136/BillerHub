...
const UI_Notes = {
  ...
  render(notes, activeCategory) {
    ...
    // Create category buttons (tabs) including a dynamic "ALL" option
    ...
    container.innerHTML = `
      ...
      <div class="notes-content" role="tabpanel"></div>
    `;
  ...
  renderContent(notes, activeCategory) {
    ...
    if (isAll) {
      ...
      finalHtml = Object.values(notes).map(note => note.content).join('');
    } else if (notes[activeCategory]) {
      // Otherwise, show the content for the specific category
      finalHtml = notes[activeCategory].content;
    }
    
    const categoryColor = isAll ? 'primary' : notes[activeCategory]?.color || 'secondary';
