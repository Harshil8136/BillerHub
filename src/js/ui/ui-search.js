...
UI.Search = {
  ...
  renderSuggestions: function(suggestions, query) {
    ...
    const queryRegex = new RegExp(`(${esc(query)})`, 'ig');
    
    const renderItem = (biller, index) => {
      const highlightedName = biller.name.replace(queryRegex, `<mark>$1</mark>`);
      const statusIndicator = `<span class="status-dot ${biller.live ? 'is-live' : 'is-offline'}"></span>`;
    ...
    dom.suggestionsList.innerHTML = suggestions.map(renderItem).join('');
