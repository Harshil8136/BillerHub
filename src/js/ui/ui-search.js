 ...
 * UI SEARCH
 * ==================
 * FINAL FIX: Removed the erroneous call to `clearSuggestions()` from within
 * `renderSuggestions()`. This was the root cause of the navigation and
 * auto-selection bug, as it was clearing the application state immediately
 * after it had been populated.
 * This module controls the UI for the main search combobox.
 */
...
UI.Search = {
   ...
   */
  init: function() {
    // Idempotency guard to prevent double-binding listeners.
  ...
  renderSuggestions: function(suggestions, query) {
    // If there are no suggestions, clear the list from the UI and exit.
    // This prevents the state from being cleared prematurely when it should be rendered.
    ...
    // By using setTimeout(0), we push the highlighting to the next tick of
    // the event loop, after the browser has painted the new DOM elements.
    setTimeout(() => this.updateActiveSuggestion(), 0);
