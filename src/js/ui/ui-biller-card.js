...
UI.BillerCard = {
  ...
  display: function(biller) {
    ...
    const createContactItem = (contact) => {
      ...
      const annotationHtml = contact.annotation ? `<span class="contact-annotation">(${contact.annotation})</span>` : '';
      ...
      return `
        <div class="contact-grid-item">
          <div class="contact-label"><i class="${iconClass} fa-fw"></i> ${contact.label} ${annotationHtml}</div>
          <div class="contact-value">${valueHtml}</div>
        </div>`;
    };

    const groupedContacts = biller.contacts?.reduce((acc, contact) => {
      if (!contact.group) return acc; // Skip contacts without a group
      (acc[contact.group] = acc[contact.group] || []).push(contact);
      return acc;
    }, {});
    
    const contactsHtml = groupedContacts
      ? Object.entries(groupedContacts).map(([groupName, contacts]) => {
          const isInternalGroup = contacts.some(c => c.type === 'Internal');
          const groupClass = isInternalGroup ? 'contact-group-internal' : '';
          const noteHtml = isInternalGroup ? '<span class="internal-badge">Internal Use Only</span>' : '';
