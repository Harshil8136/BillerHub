 ...
 * APP MAIN
 * ==================
 * UPDATED: Corrected a bug in the 'Clear Cache & Reset' button's
 * event listener to ensure the reset function is properly called.
 *
...
function init() {
    try {
        // CRITICAL ORDER: Templates must be injected into the DOM first.
        UI.Templates.inject();
        // CRITICAL ORDER: DOM elements from templates can only be cached after injection.
        cacheDOMElements(); 

        handleTelemetryConsent();
        
        // Initialize UI modules that depend on cached DOM elements.
        UI.Popovers.init();
        UI.Search.init();
        UI.FeeTool.init();
        UI.ToolMenu.init();
        ...
        // Diagnostic check for search functionality.
        if (!searchService) {
            console.warn("Search service could not be initialized. Search will not function.");
            UI.Notifications.show("Search functionality is currently unavailable.", "error");
        }
        
        initializeTitleObserver();
        
        // CRITICAL ORDER: Attach event listeners last, after all components are ready.
...
function cacheDOMElements() {
    ...
    dom.clockPopover = document.getElementById('clockPopover');
    ...
    dom.shortcutsCloseBtn = document.getElementById('shortcutsCloseBtn');
    dom.globalTooltip = document.getElementById('globalTooltip');
}
